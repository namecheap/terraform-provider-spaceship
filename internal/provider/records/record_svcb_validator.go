...
type svcbRecordValidator struct{}

var _ validator.Object = &svcbRecordValidator{}

func SVCBValidator() validator.Object {
	return &svcbRecordValidator{}
}

func (v *svcbRecordValidator) Description(_ context.Context) string {
	return "validates that SVCB records contain all required fields: svc_priority, target_name, and svc_params; and that port and scheme are well-formed when set"
}

func (v *svcbRecordValidator) MarkdownDescription(ctx context.Context) string {
	return v.Description(ctx)
}

// ValidateObject checks SVCB record-specific fields only.
// Name and TTL are validated by schema-level attribute validators in the resource schema.
func (v *svcbRecordValidator) ValidateObject(ctx context.Context, req validator.ObjectRequest, resp *validator.ObjectResponse) {
	...
	if typeAttr.ValueString() != "SVCB" {
		return
	}

	rec := &clientrecords.SVCBRecord{}
	...
	if !ok {
		resp.Diagnostics.AddAttributeError(
			...
			"Invalid Field Type",
			"The 'svc_priority' field must be an integer for SVCB records.",
		)
	} else if svcPriorityAttr.IsNull() || svcPriorityAttr.IsUnknown() {
		resp.Diagnostics.AddAttributeError(
			req.Path.AtName("svc_priority"),
			"Missing Required Field",
			"The 'svc_priority' field is required for SVCB records.",
		)
	} else {
	...
	if !ok {
		resp.Diagnostics.AddAttributeError(
			...
			"Invalid Field Type",
			"The 'target_name' field must be a string for SVCB records.",
		)
	} else if targetNameAttr.IsNull() || targetNameAttr.IsUnknown() {
		resp.Diagnostics.AddAttributeError(
			req.Path.AtName("target_name"),
			"Missing Required Field",
			"The 'target_name' field is required for SVCB records.",
		)
	} else {
	...
	if !ok {
		resp.Diagnostics.AddAttributeError(
			...
			"Invalid Field Type",
			"The 'svc_params' field must be a string for SVCB records.",
		)
	...
	if !ok {
		resp.Diagnostics.AddAttributeError(
			...
			"Invalid Field Type",
			"The 'port' field must be a string for SVCB records.",
		)
	...
	if !ok {
		resp.Diagnostics.AddAttributeError(
			...
			"Invalid Field Type",
			"The 'scheme' field must be a string for SVCB records.",
		)
